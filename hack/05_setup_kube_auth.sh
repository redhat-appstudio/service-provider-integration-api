...
kubectl exec -ti vault-0 -n vault -- sh -c 'vault policy write vault-spi-policy /tmp/policy.webapp.hcl'
kubectl exec -ti vault-0 -n vault -- sh -c 'vault write auth/kubernetes/role/vault-spi-role \
